                ...
                ScrollView {
                    LazyVStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                                ...
                                HStack(spacing: 12) {
                                    ForEach(GameMode.allCases) { mode in
                                        Button {
                                            ...
                                            VStack(spacing: 8) {
                                                Image(systemName: mode == .noteIdentifier ? "music.note" : mode == .chordQuiz ? "guitars.fill" : "waveform.path")
                                                    .font(.title2)
                            ...
                            // XP summary
                            if gameScore.totalXP > 0 {
                                HStack(spacing: 6) {
                                    Image(systemName: "bolt.fill")
                                        .foregroundColor(.orange)
                                    Text("Nivel \(gameScore.level) · \(gameScore.totalXP) XP")
                                        .font(.caption.bold())
                                        .foregroundColor(.orange)
                                    if gameScore.streak > 0 {
                                        Text("· 🔥 \(gameScore.streak)")
                                            .font(.caption)
                                    }
                                }
                                .padding(.top, 2)
                        ...
                        ForEach(filtered) { lesson in
                            LessonRow(lesson: lesson,
                                      completed: gameScore.completedLessonIDs.contains(lesson.id))
                                .onTapGesture {
                                    HapticManager.tap()
            ...
            Group {
                if showCompletion {
                    LessonCompletionView(lesson: lesson) {
                        gameScore.completeLesson(id: lesson.id, xp: lesson.xpReward)
