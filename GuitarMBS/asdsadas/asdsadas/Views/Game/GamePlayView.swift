...
struct GamePlayView: View {
    ...
    var body: some View {
                    ...
                    Button("Salir") {
                        ...
                        if secs >= 5 {
                            practiceHistory.addSession(
                                type: .game, duration: secs,
                                details: "\(mode.rawValue) · \(vm.correct) correctas")
                        }
                        dismiss()
                    }
                }
        ...
        .onChange(of: vm.correct) {
            if vm.correct >= 50 { achievements.unlock("game-50-correct") }
        }
        .onChange(of: vm.streak) {
            if vm.streak >= 5  { achievements.unlock("streak-5") }
            if vm.streak >= 10 { achievements.unlock("streak-10") }
        }
    }
